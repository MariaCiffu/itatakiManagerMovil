...
export const getJugadoresConMultas = async () => {
  try {
    console.log("📡 Cargando jugadores con multas pendientes...");

    const teamId = await getCurrentTeamId();
    const multasRef = collection(db, COLLECTIONS.MULTAS);

    // Obtener todas las multas pendientes del equipo
    const q = query(
      multasRef,
      where("teamId", "==", teamId),
      where("paid", "==", false)
    ...
    snapshot.forEach((doc) => {
      const multa = doc.data();
      if (!multasGroup[multa.playerId]) {
        multasGroup[multa.playerId] = [];
      }
      multasGroup[multa.playerId].push({
        id: doc.id,
        ...multa,
      });
    });

    // Obtener datos de jugadores y calcular totales
    const jugadoresConMultas = [];

    for (const playerId of Object.keys(multasGroup)) {
      try {
        const jugador = await getJugadorById(playerId);
        if (jugador) {
          const multas = multasGroup[playerId];
          const totalPendiente = multas.reduce(
            (sum, multa) => sum + multa.amount,
            0
          );

          jugadoresConMultas.push({
            ...jugador,
            multasPendientes: multas.map((multa) => ({
              reason: multa.reason,
              amount: multa.amount,
              date: multa.date || new Date().toLocaleDateString(),
            })),
            totalPendiente,
          });
        }
      } catch (error) {
        console.error(`Error cargando jugador ${playerId}:`, error);
      }
    }

    console.log("✅ Jugadores con multas cargados:", jugadoresConMultas.length);
    return jugadoresConMultas;
  } catch (error) {
    console.error("❌ Error en getJugadoresConMultas:", error);
    return [];
  }
};
