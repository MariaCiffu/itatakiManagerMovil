...
export function lineupReducer(state, action) {
  switch (action.type) {
    ...
    case ACTIONS.ASSIGN_PLAYER:
      ...
      const newSubstitutes = state.substitutes.filter(
    ...
    case ACTIONS.ADD_SUBSTITUTE:
      ...
      const isInLineup = Object.values(state.lineup).some(
        (p) => p?.id === action.payload.id
      );
      const isInSubstitutes = state.substitutes.some(
        (p) => p.id === action.payload.id
      );
    ...
    case ACTIONS.REASSIGN_PLAYERS: {
      const { newFormation } = action.payload;
