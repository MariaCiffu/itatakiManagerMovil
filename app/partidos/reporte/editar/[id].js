// app/partidos/reporte/editar/[id].js - Para EDITAR reporte existente
...
export default function EditarReporteScreen() {
  ...
  // Estado del reporte - se inicializará con datos existentes
  ...
  const loadData = async () => {
    try {
      ...
      // Verificar que tenga un reporte completado
      if (!partidoData.reportePartido?.completado) {
        Alert.alert("Error", "Este partido no tiene un reporte para editar");
        router.back();
        return;
      }
      ...
      // Cargar datos existentes del reporte
      setReporteData({
        resultado: partidoData.reportePartido.resultado || {
          golesLocal: 0,
          golesVisitante: 0,
        },
        jugadores: partidoData.reportePartido.jugadores || [],
      });
    } catch (error) {
  ...
  const proceedToSave = async () => {
    try {
      ...
      const result = await updateReportePartido(id, reporteData);

      if (result.success) {
        Alert.alert(
          "Reporte actualizado",
          "El reporte del partido se ha actualizado correctamente",
          [
            {
              text: "Ver reporte",
        ...
        <View style={styles.section}>
          ...
          {reporteData.jugadores && reporteData.jugadores.length > 0 ? (
            reporteData.jugadores
              .filter((jugador) => jugador && jugador.playerId)
              .map((jugador, index) => (
            ...
            <View style={styles.noJugadoresContainer}>
              <Text style={styles.noJugadoresText}>
                No hay jugadores en el reporte
              </Text>
            </View>
          )}
        </View>
          ...
          >
            {saving ? (
              ...
              <>
                ...
                <Text style={styles.saveButtonText}>Actualizar reporte</Text>
              </>
            )}
          </LinearGradient>
        </TouchableOpacity>
